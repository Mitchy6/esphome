 ...
 */
inline static void epd6::fast_gpio_set_hi(gpio_num_t gpio_num) {
  GPIO.out_w1ts = (1 << gpio_num);
}

inline static void epd6::fast_gpio_set_lo(gpio_num_t gpio_num) {
  GPIO.out_w1tc = (1 << gpio_num);
}

void epd6::busy_delay(uint32_t cycles) {
  volatile unsigned long counts = XTHAL_GET_CCOUNT() + cycles;
  while (XTHAL_GET_CCOUNT() < counts) {
  };
}

inline static void epd6::push_cfg_bit(bool bit) {
...
void epd6::push_cfg(epd_config_register_t *cfg) {
...
float epd6::get_setup_priority() const { return setup_priority::PROCESSOR; }
size_t epd6::get_buffer_length_() {
  if (this->greyscale_) {
...
void HOT epd6::draw_absolute_pixel_internal(int x, int y, Color color) {
...
void epd6::clean_fast_(uint8_t c, uint8_t rep) {
